...
| Tools     | `node/`    | Node 20, Fastify, Playwright                      | Deterministic browser + provider tools. Called by agent over HTTP.  |
| Portal    | `portal/`  | React 18, Vite, TypeScript, Tailwind, shadcn/ui   | Review UI. Talks only to the agent API.                             |

## Architecture rules

1. **Agent is the brain.** All reasoning, ranking, drafting, and workflow orchestration lives in `Agent/`.
2. **Node is a dumb tool service.** It drives Chrome and parses provider DOMs. It never calls agent back.
3. **Portal only talks to agent**, at `http://127.0.0.1:8005/api/...`. The portal never knows Node exists.
