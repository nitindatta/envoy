...
class IndeedDriftError(Exception):
    def __init__(self, drift: ToolDrift) -> None:
        super().__init__(f"Indeed parser drift: {drift.parser_id}")
        self.drift = drift


class IndeedToolError(Exception):
    def __init__(self, error: ToolError) -> None:
        super().__init__(f"Indeed tool error: {error.type}: {error.message}")
        self.error = error
...
async def search_indeed(
    ...
    *,
    ...
    envelope = await client.call("/tools/providers/indeed/search", payload)

    if envelope.status == "drift":
        assert envelope.drift is not None
        raise IndeedDriftError(envelope.drift)
    if envelope.status == "error":
        assert envelope.error is not None
        raise IndeedToolError(envelope.error)
    if envelope.status != "ok" or not isinstance(envelope.data, dict):
        raise IndeedToolError(
            ToolError(type="unexpected_envelope", message=f"status={envelope.status}")
        )

    raw_jobs = envelope.data.get("jobs", [])
    if not isinstance(raw_jobs, list):
        raise IndeedToolError(ToolError(type="bad_payload", message="jobs is not a list"))
